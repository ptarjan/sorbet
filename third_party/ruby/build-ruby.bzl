...
# Create a wrapper script to work around the toolchain's cc_wrapper.sh incorrectly treating
# macOS special linker paths (@executable_path, @loader_path, @rpath) as response files.
# The wrapper detects these special paths and calls clang directly instead of cc_wrapper.sh.
cc_wrapper_dir="$(mktemp -d)"
real_cc="$base/{cc}"

# Extract the direct clang path from the wrapper or find it in the same directory
wrapper_dir="$(dirname "$real_cc")"
if [[ -f "$wrapper_dir/clang" ]]; then
    direct_clang="$wrapper_dir/clang"
else
    # Try to extract the clang path from cc_wrapper.sh
    direct_clang=$(grep -o '/[^"]*llvm[^"]*/bin/clang' "$real_cc" 2>/dev/null | head -1 || echo "")
    if [[ -z "$direct_clang" ]] || [[ ! -f "$direct_clang" ]]; then
        # Last resort: use the wrapper and hope for the best
        direct_clang="$real_cc"
    fi
fi

cat > "$cc_wrapper_dir/cc" << WRAPPER_EOF
#!/bin/bash
set -euo pipefail

real_cc="$real_cc"
direct_clang="$direct_clang"

# Check if any argument contains macOS special paths
has_macos_paths=false
for arg in "\\$@"; do
    if [[ "\\$arg" == *@executable_path* ]] || [[ "\\$arg" == *@loader_path* ]] || [[ "\\$arg" == *@rpath* ]]; then
        has_macos_paths=true
        break
    fi
done

if [[ "\\$has_macos_paths" == true ]]; then
    # Bypass cc_wrapper.sh's broken response file handling by calling clang directly
    exec "\\$direct_clang" "\\$@"
else
    # Use the normal wrapper for everything else
    exec "\\$real_cc" "\\$@"
fi
WRAPPER_EOF
...
export CC="$cc_wrapper_dir/cc"
export CFLAGS="{copts}"
export CXXFLAGS="{copts}"
export CPPFLAGS="{sysroot_flag} ${{inc_path[*]:-}} {cppopts}"
# Point pkg-config to OpenSSL 1.1 to avoid using OpenSSL 3 which is incompatible with Ruby 2.7
if [[ -d /usr/local/opt/openssl@1.1/lib/pkgconfig ]]; then
    export PKG_CONFIG_PATH="/usr/local/opt/openssl@1.1/lib/pkgconfig:${{PKG_CONFIG_PATH:-}}"
elif [[ -d /opt/homebrew/opt/openssl@1.1/lib/pkgconfig ]]; then
    export PKG_CONFIG_PATH="/opt/homebrew/opt/openssl@1.1/lib/pkgconfig:${{PKG_CONFIG_PATH:-}}"
fi
# Build LDFLAGS, avoiding double spaces when lib_path is empty
ldflags_parts=("{sysroot_flag}")
if [[ ${{#lib_path[@]}} -gt 0 ]]; then
  ldflags_parts+=("${{lib_path[@]}}")
fi
ldflags_parts+=("{linkopts}")
...
# Try to update rubygems, but don't fail if it doesn't work (compatibility issues)
cp "$base/{rubygems}" rubygems-update.gem
if gem install --no-document --local --env-shebang rubygems-update.gem >> build.log 2>&1; then
    ...
    echo "Warning: rubygems update failed, continuing with bundled version"
fi
...
# --local to avoid going to rubygems for the gem
# --env-shebang to not hardcode the path to ruby in the sandbox
# Allow failure due to potential gem checksum issues with Ruby 2.7
if ! gem install --local --env-shebang package.gem >> build.log 2>&1; then
    echo "Warning: gem install failed for {file}, continuing anyway"
fi
...
def _build_ruby_impl(ctx):
        ...
        for linker_input in cc_info.linking_context.linker_inputs.to_list():
            for lib in linker_input.libraries:
                if lib.dynamic_library:
                    lib_path = lib.dynamic_library.path
                    # Skip Bazel-internal _solib_ directories - these are created for
                    # cc_import and point to system libraries that don't need explicit -L paths
                    if "_solib_" not in lib_path:
                        lib_dir = lib_path[:lib_path.rfind("/")]
                        libs[lib_dir] = True
    ...
    # Filter out -mlinker-version flag as it causes issues with Ruby's configure script
    # on modern macOS with newer linkers
    filtered_linkopts = [opt for opt in ctx.attr.linkopts if not opt.startswith("-mlinker-version")]

    # Build
    ctx.actions.run_shell(
        mnemonic = "BuildRuby",
        inputs = depset(
            direct = deps + ctx.files.src + ctx.files.rubygems + ctx.files.libyaml + ctx.files.libffi + ctx.files.gems + ctx.files.extra_srcs + ctx.files.append_srcs,
        ...
        command = ctx.expand_location(_BUILD_RUBY.format(
            ...
            linkopts = " ".join(filtered_linkopts),
        ...
        tools = depset(
            direct = [ctx.toolchains["@rules_rust//rust:toolchain_type"].rustc],
            transitive = [cc_toolchain.all_files],
        ),
    )
