...
ruby="./bazel-bin/external/sorbet_ruby_3_0_for_compiler/toolchain/bin/ruby"
...
if [ -n "$debug" ]; then
  if [ -n "$use_gdb" ]; then
    ./bazel build @sorbet_ruby_3_0_for_compiler//:ruby --config dbg --config=static-libs
  ...
  ./bazel build @sorbet_ruby_3_0_for_compiler//:ruby -c opt
  command=( "${ruby}" )
fi
